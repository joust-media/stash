...
export async function insertTransaction(
  ...
  const [result] = await db.query(
    `INSERT INTO transactions
       (kid_id, type, amount_cents, note, category, related_completion_id, created_by, created_at, balance_after_cents)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
    [
      t.kidId,
      t.type,
...
export async function goalsFor(kidId: number, balanceCents: number, db: Db = pool): Promise<Goal[]> {
  const rows = await rows_<GoalRowShape>(db, 'SELECT * FROM goals WHERE kid_id = ? ORDER BY active DESC, id', [
    kidId,
  ])
...
export async function goalFor(kidId: number, balanceCents: number, db: Db = pool): Promise<Goal | null> {
  const row = await first<GoalRowShape>(
    db,
    'SELECT * FROM goals WHERE kid_id = ? AND active = 1 ORDER BY id LIMIT 1',
    [kidId],
  )
